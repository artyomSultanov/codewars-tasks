...
const findPowOf2 = (n) => {
  return n
    ...
    .reduce(
      (acc, elem, index, arr) =>
        elem === '1' ? acc.concat(2 ** (arr.length - 1 - index)) : acc,
      []
    )
}
...
function fib(n) {
  ...
  if (n > 0) {
    ...
    const matPowers = findPowOf2(n).map((elem) =>
      getMatPower(M, elem, memoPowers)
    )
    let res = matPowers[0]
...
function makeHuffmanTree(freqs) {
  ...
  while (freqs.length !== 1) {
    ...
    let right = freqs.pop()
    let left = freqs.pop()
    let [rightName, rightFreq] = Array.isArray(right)
      ? [right[0], right[1]]
      : [right.value[0], right.value[1]]
    let [leftName, leftFreq] = Array.isArray(left)
      ? [left[0], left[1]]
...
function encode(freqs, s) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    const makeCodeInTree = (sym, tree, code) => {
      ...
      if (tree.right['value'] === sym || tree.right[0] === sym)
        return code + '1'
    }
...
function decode(freqs, bits) {
  ...
  const huffmanTree = Array.isArray(freqs[0])
    ? makeHuffmanTree(freqs)
    : freqs[0]
  let result = ''
  for (let i = 0; i < bits.length; i++) {
    const makeStringInTree = (bit, tree) => {
      if (bit === '0' && Array.isArray(tree.left) && tree.left[0].length === 1)
        return tree.left[0]
      if (
        bit === '1' &&
        Array.isArray(tree.right) &&
        tree.right[0].length === 1
      )
        return tree.right[0]
      if (bit === '0' && !Array.isArray(tree.left))
        return makeStringInTree(bits[++i], tree.left)
      if (bit === '1' && !Array.isArray(tree.right))
        return makeStringInTree(bits[++i], tree.right)
    }
