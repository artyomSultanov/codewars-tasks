...
function rot13(message) {
  return message
    ...
    .map((char) => {
      ...
      return !~char.search(/[A-Za-z]/)
        ? char
        : String.fromCharCode(
            ((char.charCodeAt(0) + 13 - caseChar) % 26) + caseChar
          )
    })
    .join('')
}
...
// O(n*m) where m === Math.max(...words.map((w) => w.length))
function anagrams(word, words) {
  ...
  words.forEach((w) => {
    ...
    for (let i = w.length - 1; i > -1; i--) {
      const charCount = wMap.get(w[i])
      if (!wordMap.has(w[i])) break
      if (!charCount) wMap.set(w[i], 1)
      if (charCount) wMap.set(w[i], wMap.get(w[i]) + 1)
    }

    if (wMap.size !== wordMap.size) return

    for (const [key, value] of wordMap) {
      if (value !== wMap.get(key)) return
    }
